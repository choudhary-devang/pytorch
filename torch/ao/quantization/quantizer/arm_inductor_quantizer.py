...
__all__ = [
    ...
    "get_arm_inductor_linear_dynamic_fp16_config",
]
...
# try to change this key and see what is happening with this!!!
    ...
    for node in nodes:
        if node is not None:
            if QUANT_ANNOTATION_KEY not in node.meta:
                node.meta[QUANT_ANNOTATION_KEY] = _ArmInductorQuantizationAnnotation()
...
def _is_quantized_op_pt2e(node: torch.fx.Node):
    ...
    assert isinstance(quantization_annotation, _ArmInductorQuantizationAnnotation)
...
@functools.lru_cache
def get_arm_inductor_linear_dynamic_fp16_config():
    """
...
class ArmInductorQuantizer(Quantizer):
    ...
    def _need_skip_config(
        ...
        """Check if the provided quantization config is valid for ArmInductorQuantizer.
    ...
    def _annotate_conv_node_helper(
        ...
        if annotate_output:
            conv_node.meta[QUANT_ANNOTATION_KEY] = _ArmInductorQuantizationAnnotation(
    ...
    def _annotate_linear_node_helper(
        ...
        if annotate_output:
            linear_node.meta[QUANT_ANNOTATION_KEY] = _ArmInductorQuantizationAnnotation(
                input_qspec_map=input_qspec_map,
                _annotated=True,
                _is_output_of_quantized_pattern=True,
            )
    ...
    def _annotate_matmul(
        ...
        for node in model.graph.nodes:
            ...
            for input_node in matmul_node.args:
                input_qspec_map[input_node] = get_input_act_qspec(quantization_config)
            matmul_node.meta[QUANT_ANNOTATION_KEY] = _ArmInductorQuantizationAnnotation(
                input_qspec_map=input_qspec_map,
                _annotated=True,
                _is_output_of_quantized_pattern=True,
            )
    ...
    def _annotate_conv2d_binary_unary(
        ...
        for fused_partition in fused_partitions:
            ...
            binary_node.meta[QUANT_ANNOTATION_KEY] = _ArmInductorQuantizationAnnotation(
                input_qspec_map=binary_node_input_qspec_map,
                _annotated=True,
            )
            unary_node.meta[QUANT_ANNOTATION_KEY] = _ArmInductorQuantizationAnnotation(
                _annotated=True,
                _is_output_of_quantized_pattern=True,
            )
    ...
    def _annotate_maxpool2d(
        ...
        input_qspec_map = {}
        input_qspec_map[input_node] = get_input_act_qspec(quantization_config)
        maxpool_node.meta[QUANT_ANNOTATION_KEY] = _ArmInductorQuantizationAnnotation(
            input_qspec_map=input_qspec_map,
            _annotated=True,
            _is_output_of_quantized_pattern=True,
        )
    ...
    def _annotate_cat(
        ...
        cat_node.meta[QUANT_ANNOTATION_KEY] = _ArmInductorQuantizationAnnotation(
            input_qspec_map=input_qspec_map,
            _annotated=True,
            _is_output_of_quantized_pattern=True,
        )
